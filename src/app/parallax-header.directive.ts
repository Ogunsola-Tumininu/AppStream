...
export class ParallaxHeaderDirective implements AfterViewInit {
  ...
  // disable-next-line:no-input-rename
  parallaxHeight = 300;
  ...
  ngAfterViewInit() {
    ...
    //   this.renderer.setStyle(this.header, 'background-image', 'url(../../assets/img/newsfeed.jpg)');
    ...
    this.domCtrl.read(() => {
      ...
      this.domCtrl.write(() => {
        this.renderer.setStyle(this.header,
          'transform', 'translate3d(0,' + translateAmt + 'px,0) scale(' + scaleAmt + ',' + scaleAmt + ')');
